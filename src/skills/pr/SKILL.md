...
It prints `storage=` (linear|local — @@ship/patterns/storage-mode.md is the definition), `branch=`/`on_default_branch=`, `remote=`/`pr_base=` (pull, push and `--base` target these — never a hardcoded `origin`/`main`), `keep_context=` (`yes` = work-graph node: the graph polls this scratch dir and watches this PR's state on the forge), `pending_changes=`, `profile=`, `approval=` (local marker check; `unknown` in Linear — check cached comments), and one `gate_row:` per phase when the scratch dir exists.
...
```bash
bash "@@ship/hooks/pr-sync.sh" --remote <remote> --base <pr_base>
```
