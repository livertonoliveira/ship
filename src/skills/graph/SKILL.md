...
`bash "@@ship/hooks/graph.sh" status` renders the graph at any point; `--json` gives the raw state. `graph-log.md` in the graph dir carries the running timeline — every claim, poll, seal and merge — and is the only progress signal visible from outside this turn.
