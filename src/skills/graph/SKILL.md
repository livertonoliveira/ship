...
To stop a run, `graph.sh abort`: it stops each in-flight worker, marks those nodes failed and keeps their workspaces. Killing the orchestrator does NOT do this — workers survive it and keep billing. `bash "@@ship/hooks/graph.sh" reset <task>... | --all` returns failed nodes to pending, each retry in a fresh workspace.
