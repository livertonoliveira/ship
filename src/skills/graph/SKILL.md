...
`ship/config.md` must exist (else `/ship:init` + STOP); storage mode @@ship/patterns/storage-mode.md. Driver defaults to `manual`; `orca` needs that CLI on PATH, `local` needs nothing.
