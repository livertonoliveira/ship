...
Parse `$ARGUMENTS`: `task-id` = first token (standalone → derive from branch name or `standalone`). Parse `Mode:` — `generate`/`execute`/`full`, default `full`. Layers = `bash "@@ship/hooks/test-scope.sh" --config ship/config.md` (`run=`/`skip=`, deterministic — never compute in prose). Read `Artifact language`; read `stack.md` (fallback `ship/config.md`).
