...
- **Spec quality:** audit REQ/AC for ambiguity (AMBIG), underspecification (SUBSPEC), convention violations (PRINCIPLE) via `@@ship/patterns/spec-quality.md` — pre-filters + one batched sub-agent, strict JSON; apply rewrites first. Runs only here, once — never inside the pipeline (`/ship:run`).
