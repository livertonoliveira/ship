...
**(b) Quality:** class→agent-set scope (deterministic, never in prose) — `bash "@@ship/hooks/quality-scope.sh" <class> --phases "<enabled quality phases>" --scratch .context/ship-run/<task-id>` (`<class>` from `diff-class.txt`): writes PASS skip rows for skipped phases, prints `run=`/`log=`. Pre-quality snapshot captured (step 0).
