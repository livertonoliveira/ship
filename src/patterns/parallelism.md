...
- **Parallel (the only allowed fan-outs):**
  - Quality stage in `/ship:run`: `perf` + `security` + `review` dispatched as Agents in one turn (plus each worker's internal sub-agents) — read-only diff analysis under independent lenses.
