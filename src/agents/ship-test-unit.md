...
- Name the test by the **observable behavior** it asserts (e.g., `"ignores duplicate event for same transactionId"`). **NEVER** prefix or suffix the test name with spec IDs (`SC-XX`, `AC-XX`, `REQ-XX`, `MOB-XXXX`) and **NEVER** add `// TEST-SC-XX` (or any) marker comments — no comments of any kind in test files.
