...
> **MANDATORY — LINEAR MODE ONLY.** Never pass literal `"In Progress"` — it no-ops on teams with a differently-named started state. Read `${CLAUDE_SKILL_DIR}/patterns/linear-status.md`, follow that recipe, then `mcp__linear-server__save_issue` with `state: <target-state>` before dispatching any worker.
