...
- **Local mode:** relative path `ship/changes/<feature>/report-<task-id>.md` and render per # Ship — Report Templates
...
- Low: X
- **Gate: PASS | WARN | FAIL**
```#quality-report. Keep the consolidated report **in memory** for reuse in the PR body (do not rely on re-reading it later).
...
2. **Record approval + close the issue:**
   - **Linear mode** — in parallel:
     - `mcp__linear-server__save_comment`: post the consolidated quality report as a comment, with the Homologation section set to:
       ...
       ```
     - `mcp__linear-server__save_issue`: transition the issue to its completed state. **Do not pass the literal `"Done"`** — resolve the team's completed-state name per # Linear Status — resolve, set, and verify workflow-state transitions
...
For the exact rendering format (Lazy Mode — PASS table vs WARN/FAIL expanded block), see # Ship — Report Templates
...
- Low: X
- **Gate: PASS | WARN | FAIL**
```#pr-body for the PR body template.
