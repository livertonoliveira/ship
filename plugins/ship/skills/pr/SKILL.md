...
Storage mode: ${CLAUDE_SKILL_DIR}/patterns/storage-mode.md.
...
- **Post-homolog:** approval marker `- [x] User approves for PR` exists (Linear: cached comments; Local: `report.md`'s Homologation section). Continue to Prerequisite 2.
...
1. Consolidate the quality report in memory (reused in the PR body, don't re-read later): get the per-phase gate index with `bash "${CLAUDE_SKILL_DIR}/hooks/pipeline.sh" rows .context/ship-run/<task-id>` (most recent row per phase — never re-derive it by hand); for WARN/FAIL phases apply ${CLAUDE_SKILL_DIR}/patterns/lazy-load-findings.md to the findings file. Render via the PR Body Template's Quality Report structure (${CLAUDE_SKILL_DIR}/report-templates.md) plus `## Fixes Applied` (list or "None.") and a Homologation checklist. Missing scratch dir: note gate data unavailable, proceed anyway.
2. Record approval + close the issue:
   - **Linear** (parallel): `save_comment` with the report, Homologation set to `- [x] User has reviewed all changes / - [x] User has verified acceptance criteria / - [x] User approves for PR — Approved on YYYY-MM-DD (direct /ship:pr)`; transition via the full recipe in ${CLAUDE_SKILL_DIR}/patterns/linear-status.md (resolve → `save_issue` → verify `state.type == "completed"` → retry once; never pass literal `"Done"`; surface, don't proceed silently). Re-fetch `list_comments` once, replacing the cache.
...
**Linear:** if `.context/ship-run/<task-id>/linear-cache.json` exists, `get_document` per cached id (Proposal/Design), skipping `list_documents` (log `cached_at`, non-gating). Otherwise use ${CLAUDE_SKILL_DIR}/patterns/load-artifacts.md. Reuse cached `list_comments` from Prerequisites — do not re-fetch.

**Local:** ${CLAUDE_SKILL_DIR}/patterns/load-artifacts.md, plus `tasks.md` (completeness) and `report.md` (gates/findings).
...
Reuse the current branch if it's not `main`/`master`.
...
Read `Pipeline Profile → profile` from `ship/config.md`. Skip unless `profile: strict`.
...
Apply the lazy-load algorithm at ${CLAUDE_SKILL_DIR}/patterns/lazy-load-findings.md per phase (perf, security, review). **Linear:** findings come from the cached quality-report comment (from Prerequisites); link = that comment's URL. **Local:** read `ship/changes/<feature>/report-<task-id>.md`; path = same file.

Build the body from the loaded artifacts following the PR Body Template section of ${CLAUDE_SKILL_DIR}/report-templates.md, then create via HEREDOC:
...
If `--keep-context` was passed, skip and log: "Scratch dir preserved for inspection: `.context/ship-run/<task-id>/`". Otherwise `rm -rf .context/ship-run/<task-id>` (never the parent `ship-run/` dir — other pipelines may use it) and log: "Scratch dir cleaned up."

### 10. Archive (Local mode only)

```bash
mv ship/changes/<feature-name> ship/changes/archive/$(date +%Y-%m-%d)-<feature-name>
```

Linear: nothing to archive — artifacts stay in Linear.
