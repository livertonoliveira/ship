...
It prints `storage=` (linear|local — ${CLAUDE_SKILL_DIR}/patterns/storage-mode.md is the definition), `branch=`/`on_default_branch=`, `pending_changes=`, `profile=`, `approval=` (local marker check; `unknown` in Linear — check cached comments), and one `gate_row:` per phase when the scratch dir exists.
...
```bash
bash "${CLAUDE_SKILL_DIR}/hooks/pr-finalize.sh" <task-id> [--keep-context] [--feature <feature-name>]
```
