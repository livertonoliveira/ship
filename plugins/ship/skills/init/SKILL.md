...
Launch **2 agents in parallel** using the Agent tool. For BOTH agents, pass `model: "sonnet"` explicitly to the Agent tool call — they read source code and infer patterns (reasoning work). The orchestrator itself runs on Sonnet per ${CLAUDE_SKILL_DIR}/patterns/model-routing.md.
...
Otherwise, read `Artifact language` from `ship/config.md → Conventions`. for conventions.
