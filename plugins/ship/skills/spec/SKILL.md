...
`linear.app` URL or `^[A-Z]+-\d+$` → Linear issue ID; else free text. # Storage Mode
...
**SC↔task cross-reference:** `bash "${CLAUDE_SKILL_DIR}/hooks/sc-crossref.sh" --index <file> --issues <dir> --proposal <file>`; fix violations, re-run until clean, before §7.
