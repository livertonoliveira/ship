...
`bash "${CLAUDE_SKILL_DIR}/hooks/graph.sh" init --feature "<project name>" --from nodes.json --driver <d> --max-in-flight <N> --mode <linear|local> [--merge-policy human|graph] [--repo <id>]`
