...
- If `frontend` → warn the user (language per # Artifact Language
...
- Low: X
- **Gate: PASS | WARN | FAIL**
```#finding-entry with the Backend audit domain extensions (Effort, Maintenance window).
...
- **Replaces `## Evidence` and appends extra fields to `## Notes`**:
  ...
  - **Current confidence:** <0.0 to 1.0>
  - **Effort:** <Hours | Days>
  ```. Use prefix `[PERF]`, label `performance`.
