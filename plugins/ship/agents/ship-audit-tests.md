...
Produce each finding per `# Ship — Report Templates
...
- Low: X
- **Gate: PASS | WARN | FAIL**
```#finding-entry` with the Test Coverage domain extensions: **Layer** (unit | integration | e2e), **Current confidence** (0.0–1.0), and **Effort** (Hours | Days). Use category `TEST`; include the closest test match (or `none`) and a Fix snippet (example test that would cover the AC/SC).
...
After writing the report, emit the audit summary JSON block per `# Audit Summary Schema
